...
```{r, echo=FALSE}
str_detect(stringr::words, "ise") |> sum()
...
```{r}
str_view(stringr::words, "(ise$|ing$)")
```
:::
...
```{r}
str_view(example.obj, "[:digit:]\\.")
str_view(example.obj, "[:alpha:]\\.")
```
:::
...
```{r}
str_view(example.obj, "1<[[:alpha:]]<6") # lol incorrect regex syntax
